...
public class GenericClassLoadingFactory<T> implements GenericFactory<T> {
    ...
    public T newInstance() throws ConfigurationException {
        ...
        try {
            Class<? extends GenericFactory<T>> clazz = (Class<? extends GenericFactory<T>>) Class.forName(clazzName);
            made = newInstance(clazz);
    ...
    private T newInstance(Class<? extends GenericFactory<T>> clazz) throws ConfigurationException {
        T made = null;
        try {
            GenericFactory<? extends T> delegate = clazz.newInstance();
            made = delegate.newInstance();
